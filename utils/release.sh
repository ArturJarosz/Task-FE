...
if [[ -n "$NEW_VERSION" && -n "$FUTURE_VERSION" ]]; then
    ...
    npm version "${FUTURE_VERSION}-SNAPSHOT" --git-tag-version false
    git add ":**\package.json"
    git add ":**\package-lock.json"
    ...
    echo " - 1.Merge release/release_$NEW_VERSION" to main branch.
    echo " - 2.Merge update/NO_JIRA_update_develop_to_$FUTURE_VERSION to develop."
fi
